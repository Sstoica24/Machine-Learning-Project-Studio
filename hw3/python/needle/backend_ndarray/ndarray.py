...
def prod(x):
    # operator is the class that performs the operation. Ex) operation.add(x,y) == x+y
    # reduces iterable to a signle value.
...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        Returns:
            NDArray : reshaped array; this will point to the previous memory
        """

        ### BEGIN YOUR SOLUTION
        prod_1 = 1
        prod_2 = 1
        for elem_1 in self._shape:
          prod_1 *= elem_1
        for elem_2 in new_shape:
          prod_2 *= elem_2
        
        if prod_1 != prod_2 or not self.is_compact():
          raise ValueError
        
        # need to return new NDArray that is of new shape, strides need to be updated
        # and there is a method compact_strides(shape) which takes in strides==> strides
        # become dependant on shape. Then, set device and handle equal to previous device 
        # and handle. 
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        # permuting order of the dims permutes shape. If you permute the shape
        # of course you must also permute the strides! 
        new_shape = []
        new_strides = []
        for ord in new_axes:
          new_shape.append(self._shape[ord])
          new_strides.append(self._strides[ord])
        new_shape = tuple(new_shape)
        new_strides = tuple(new_strides)

        # make new NDarray exactly like before
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        # shape and new shape will be same length based on how broadcasting
        # is defined
        # need to assert the lengths of the shapes are equal
        assert len(self._shape) == len(new_shape)
        for i in range(len(self._shape)):
          if self._shape[i] != 1 and self._shape[i] != new_shape[i]:
            raise AssertionError
        
        # as it says in the tensorflow documentation: "Expanding a tensor does 
        # not allocate new memory, but only creates a new view on the existing 
        # tensor where a dimension of size one is expanded to a larger size by 
        # setting the stride to 0." Stride 0 means the the next element corresponding
        # to that dimension is right next to current element.
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        # easies to use zip here
        # new_shape is given by the distances of the slides in index (no longer considering entire array)
        new_shape = [(sl.stop - sl.start + sl.step - 1) // sl.step for sl in idxs]
        #sl.start * st gives offset for each index, so to find total offset, we need to sum over all offsets
        offset = [sl.start * st for sl, st in zip(idxs, self._strides)]
        sum_off = 0
        for elem in offset:
          sum_off += elem
        # straight forward
        ...
        # like always have done. 
        return NDArray.make(new_shape, new_strides, self._device, self._handle, sum_off)
        ### END YOUR SOLUTION
